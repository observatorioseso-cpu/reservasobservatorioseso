 ...
 *   ADMIN_NOMBRE    nombre visible en el panel
 *   ADMIN_TEMPORAL  "1" marca la contraseña como temporal: el panel obliga a
 *                   cambiarla en el primer ingreso y la clave que viajó por
 *                   WhatsApp o teléfono deja de servir.
 *
...
const temporal = process.env.ADMIN_TEMPORAL === "1"
...
const admin = await prisma.admin.upsert({
  where:  { email },
  update: { passwordHash: hash, nombre, debeCambiarPassword: temporal },
  create: { email, passwordHash: hash, nombre, debeCambiarPassword: temporal },
})
...
if (temporal) {
  console.log("Marcada como TEMPORAL: el panel exigirá cambiarla en el primer ingreso.")
}
