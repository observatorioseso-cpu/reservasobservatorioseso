    ...
    reserva = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      const nueva = await tx.reserva.create({
        data: {
          ...
          idioma: data.idioma,
          cantidadPersonas: data.cantidadPersonas,
          // tienesMenores se deriva de las marcas por persona
          tienesMenores: data.titularEsMenor || data.acompanantes.some((a) => a.esMenor),
          titularEsMenor: data.titularEsMenor,
          ...
          ciudadResidencia: data.ciudadResidencia.trim(),
          infoAdicional: (data.infoAdicional ?? "").trim() || null,
          locale: data.locale,
          passwordHash,
          acompanantes: {
